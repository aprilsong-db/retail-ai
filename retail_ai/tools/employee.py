...
from retail_ai.tools.models import DepartmentInfo, TaskAssignmentInfo


def create_find_top_employees_by_department_tool(warehouse_id: str, config: ModelConfig) -> Callable:
    """
    ...
    def find_top_employees_by_department(department: str, limit: int = 5) -> tuple:
        ...
        sql_query = f"""
            SELECT 
                ...
                recognition_points,
                performance_trend
            FROM {catalog_name}.{database_name}.top_employees_by_department
...
def create_find_personal_shopping_associates_tool(warehouse_id: str, config: ModelConfig) -> Callable:
    """
    ...
    @tool
    def find_personal_shopping_associates(store_id: str = None, expertise_level: str = None) -> tuple:
        """
        ...
        Returns:
            tuple: Personal shopping associate data including:
                ...
                - availability_status
        """
        logger.debug(f"find_personal_shopping_associates: store_id={store_id}, expertise_level={expertise_level}")
        ...
        sql_query = f"""
            SELECT 
                ...
                expertise_level,
                overall_rank
            FROM {catalog_name}.{database_name}.top_personal_shopping_associates_all_stores
            {where_clause}
...
def create_department_extraction_tool(llm: LanguageModelLike) -> Callable[[str], str]:
    """
...
def create_find_employee_manager_tool(warehouse_id: str, config: ModelConfig) -> Callable:
    """
    ...
    def find_employee_manager(employee_id: str) -> tuple:
        ...
        sql_query = f"""
            ...
            FROM {catalog_name}.{database_name}.employees e
            LEFT JOIN {catalog_name}.{database_name}.employees m ON e.manager_id = m.employee_id
            LEFT JOIN {catalog_name}.{database_name}.stores s ON e.store_id = s.store_id
...
def create_task_assignment_tool(warehouse_id: str, config: ModelConfig, llm: LanguageModelLike) -> Callable:
    """
    ...
    def assign_task_to_employee(
        ...
        try:
            ...
            find_manager = create_find_employee_manager_tool(warehouse_id, config)
            ...
            insert_sql = f"""
                INSERT INTO {catalog_name}.{database_name}.task_assignments 
            ...
            if response.status.state != StatementState.SUCCEEDED:
                ...
                return {
                    ...
                    "message": f"Failed to create task assignment in database"
                }
...
def _mock_send_notification(manager_info: dict, assignment_id: str, task_title: str, task_description: str) -> dict:
    """
...
def create_task_extraction_tool(llm: LanguageModelLike) -> Callable:
    """
