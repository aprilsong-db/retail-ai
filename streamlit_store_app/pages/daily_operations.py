...
def show_store_metrics():
    ...
    departments = [
        {"name": "Women's Fashion", "sales": "$12,400", "target": "$15,000", "performance": "83%"},
        {"name": "Electronics", "sales": "$8,750", "target": "$8,000", "performance": "109%"},
        {"name": "Men's Fashion", "sales": "$4,200", "target": "$5,000", "performance": "84%"},
        {"name": "Accessories", "sales": "$3,400", "target": "$3,500", "performance": "97%"}
    ]
    ...
    for dept in departments:
        ...
        with col4:
            performance_color = "green" if float(dept['performance'].rstrip('%')) >= 100 else "orange" if float(dept['performance'].rstrip('%')) >= 80 else "red"
            st.markdown(f"<span style='color: {performance_color}'>**{dept['performance']}**</span>", unsafe_allow_html=True)
...
def show_schedule_management():
    ...
    with col1:
        ...
        current_staff = [
            {"name": "Sarah Chen", "role": "Store Associate", "department": "Women's Fashion", "shift": "8 AM - 4 PM", "status": "active"},
            {"name": "Mike Rodriguez", "role": "Store Associate", "department": "Electronics", "shift": "8 AM - 4 PM", "status": "active"},
            {"name": "Emma Wilson", "role": "Store Associate", "department": "Customer Service", "shift": "10 AM - 6 PM", "status": "active"},
            {"name": "James Park", "role": "Visual Merchandiser", "department": "All Floors", "shift": "9 AM - 5 PM", "status": "break"},
        ]
        
        for staff in current_staff:
            status_color = {"active": "#28a745", "break": "#ffc107", "off": "#6c757d"}[staff["status"]]
            st.markdown(f"""
    ...
    with col2:
        ...
        alerts = [
            {"type": "Coverage Gap", "time": "3:00 - 4:00 PM", "department": "Electronics", "severity": "high"},
            {"type": "Overtime Alert", "employee": "Sarah Chen", "hours": "42/40", "severity": "medium"},
            {"type": "Break Reminder", "employee": "Mike Rodriguez", "due": "In 30 minutes", "severity": "low"}
        ]
...
def show_priority_card(priority):
    ...
    with col3:
        if st.button("Edit Task", key=f"edit_{priority['id']}", use_container_width=True):
...
def show_delivery_card(delivery):
    ...
    with col2:
        if delivery['status'] == 'arrived':
            if st.button("Process Delivery", key=f"process_{delivery['vendor']}", use_container_width=True):
                st.success("Delivery processing started!")
        elif delivery['status'] == 'scheduled':
            if st.button("Mark Arrived", key=f"arrived_{delivery['vendor']}", use_container_width=True):
                st.success("Delivery marked as arrived!")
        else:  # delayed
            if st.button("Update ETA", key=f"update_{delivery['vendor']}", use_container_width=True):
