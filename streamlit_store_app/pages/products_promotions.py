...
def show_promotion_card(promo):
    ...
    with col2:
        if st.button(f"Share Details", key=f"share_{promo['code']}", use_container_width=True):
...
def show_product_card(item, card_type):
    ...
    with col2:
        if st.button(f"View Details", key=f"view_{item['name']}", use_container_width=True):
